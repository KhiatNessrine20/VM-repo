...
class MplsController(app_manager.RyuApp):
    ...
    def push_mpls(self, ev, out_port,pkt_ip):
        msg = ev.msg
        datapath = msg.datapath
        ofproto = datapath.ofproto
        parser = datapath.ofproto_parser
    ...
    def swap_mpls(self, ev, out_port, pkt_ip):
        msg = ev.msg
        datapath = msg.datapath
        ofproto = datapath.ofproto
        parser = datapath.ofproto_parser
    ...
    def pop_mpls(self, ev, out_port, pkt_ip):
        msg = ev.msg
        datapath = msg.datapath
        ofproto = datapath.ofproto
        parser = datapath.ofproto_parser
    ...
    def _packet_in_handler(self, ev):
        ...
        if ethtype == 2048 and dpid == "0000000000000001":
            self.push_mpls(ev, out_port, pkt_ip)
        
        if ethtype ==ether_types.ETH_TYPE_MPLS and dpid == "0000000000000003":
            self.swap_mpls(ev, out_port, pkt_ip)
        ...
        if  ethtype ==ether_types.ETH_TYPE_MPLS and dpid == "0000000000000004":
            self.pop_mpls(ev, out_port, pkt_ip)
