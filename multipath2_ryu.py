...
class SimpleRouter(app_manager.RyuApp):
    ...
    def switch_features_handler(self, ev):
        ...
        # Install the default flow entry that forwards packets to the controller
    ...
    def get_paths(self, src, dst):
        ...
        if src == dst:
            ...
            return [[src]] # if the source and destination nodes are teh same, alorsyne liste contenqnt le noeud source est retournée
        paths = [] #init d'une liste vide
        stack = [(src, [src])]  #une liste contenant a tuple with a source node and a [list only source node]
        while stack:
            (node, path) = stack.pop()
        ...
        for node in switches_in_paths:
            ...
            for in_port in ports:
                ...
                if len(out_ports) > 1:
                    ...
                    self.add_flow(dp, 32768, match_ip, actions)
                    self.add_flow(dp, 1, match_arp, actions)
                if paths_with_ports and src in paths_with_ports[0]:
    ...
    #def handle_arp(self, datapath, pkt, eth):
        arp_pkt = pkt.get_protocol(arp.arp)

        # Update the ARP table with the sender's IP and MAC addresses
