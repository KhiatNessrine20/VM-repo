...
class Router(dict):
    def __init__(self, dp, logger):
        ...
        vlan_router = VlanRouter(VLANID_NONE, dp, self.port_data, logger,  self.mpls_data)
    ...
    def packet_in_handler(self, msg):
        ...
        if header_list:
            ...
            # Event dispatch
            if vlan_id in self:
                self.logger.debug('Launching Vlan_router Packet In Handler',
...
class VlanRouter(object):
    ...
    def packet_in_handler(self, msg, header_list):
        ...
        # LER method (push/pop) a modifie accordingly
...
class OfCtl(object):
	...
	if icmp_data is None and msg_data is not None:
		...
		icmp_data = icmp.dest_unreach(data_len=len(ip_datagram),data=ip_datagram)
    ...
    def send_mpls_packet_out(self, in_port, out_port, data, label, action,dst_mac=0, src_mac=0):
        ...
        if action == MPLS_PUSH_LABEL:
            actions = [parser.OFPActionPushMpls(ethertype=34887),parser.OFPActionSetField(mpls_label=label),parser.OFPActionOutput(out_port)]
...
class OfCtl_after_v1_2(OfCtl):
    ...
    def set_my_flow(self, cookie, priority, match, idle_timeout=0, actions=None ):
        ...
        m = ofp_parser.OFPFlowMod(self.dp, cookie, 0,0,cmd, idle_timeout, 0,priority, UINT32_MAX, ofp.OFPP_ANY,ofp.OFPG_ANY,0, match,inst)       
