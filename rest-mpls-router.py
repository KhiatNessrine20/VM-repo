           ...
           # self.logger.debug('Packet in = %s', str(pkt), self.sw_id)
            ...
            # Check vlan-tag
                ...
                if vlan_id in self:
                    ...
                    self.logger.debug('Drop unknown vlan packet. [vlan_id=%d]',vlan_id, extra=self.sw_id)
...
class VlanRouter(object):
	...
	def _packetin_from_mpls_network(self, msg, header_list):
		...
		if dst_ip not in self.hosts:
			...
			self.ofctl.set_mpls_flow(cookie, priority, self.mpls_label.value,in_port, out_port, MPLS_POP_LABEL, nw_dst=dst_ip,
