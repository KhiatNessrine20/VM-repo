...
class MplsController(app_manager.RyuApp):
    ...
    def send_group_mod(self, datapath):
        ...
        LB_WEIGHT1 = 1 #percentage
        LB_WEIGHT2 = 1 #percentage

        watch_port = ofproto_v1_3.OFPP_ANY
        watch_group = ofproto_v1_3.OFPQ_ALL
        self.label = self.label + 1
        self.logger.info("Flow actions: push MPLS=%s,  dpid=%s ", self.label, dpid)
        
        actions1 = [parser.OFPActionPushMpls(ethertype=ether_types.ETH_TYPE_MPLS ),parser.OFPActionSetField(mpls_label=self.label),parser.OFPActionOutput(1)]
        actions2 = [parser.OFPActionPushMpls(ethertype=ether_types.ETH_TYPE_MPLS ),parser.OFPActionSetField(mpls_label=self.label),parser.OFPActionOutput(2)]
    ...
    def swap_mpls(self, ev, out_port):
        ...
        #actions = [parser.OFPActionPopMpls(), parser.OFPActionPushMpls(),parser.OFPActionSetField(mpls_label=self.label), parser.OFPActionOutput(2)]
        out = parser.OFPPacketOut(datapath=datapath, buffer_id=msg.buffer_id,in_port=in_port, actions=actions, data=pkt_mpls.data)
        datapath.send_msg(out)
        self.add_flow(datapath, 1, match, actions)
    ...
    def _flow_stats_reply_handler(self, ev):
        ...
        # Check if the flow key exists in the dictionary
            ...
            # Calculate the flow speed (bytes per second)
                ...
                capacity = 1 #Obtenu from port speed --1G-
